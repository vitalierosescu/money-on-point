...
export function InvoicePDF({ data }: { data: InvoiceFormData }): ReactElement {
  const subtotal = data.items.reduce((sum: number, item: InvoiceItem) => sum + item.subtotal, 0)
  const taxTotal = getInvoiceTaxAmount(data)
  const total = getInvoiceTotalAmount(data)
  const isAuthorRightsInvoice = isAuthorRightsMode(data.invoiceMode)
  const authorRightsData = buildAuthorRightsData(data)
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {isAuthorRightsInvoice && authorRightsData && (
          <View style={styles.authorRightsBox}>
            <Text style={styles.notesLabel}>Author rights split</Text>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Professional compensation</Text>
              <Text style={styles.summaryValue}>{formatCurrency(authorRightsData.professionalGrossCents, data.currency)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Author rights compensation</Text>
              <Text style={styles.summaryValue}>{formatCurrency(authorRightsData.authorRightsGrossCents, data.currency)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Service VAT</Text>
              <Text style={styles.summaryValue}>{formatCurrency(authorRightsData.serviceVatAmountCents, data.currency)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Author rights VAT</Text>
              <Text style={styles.summaryValue}>{formatCurrency(authorRightsData.rightsVatAmountCents, data.currency)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Withholding tax</Text>
              <Text style={styles.summaryValue}>-{formatCurrency(authorRightsData.withholdingAmountCents, data.currency)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Contract reference</Text>
              <Text style={styles.summaryValue}>{data.authorRightsContractReference || "\u2014"}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Agreement date</Text>
              <Text style={styles.summaryValue}>{data.authorRightsAgreementDate || "\u2014"}</Text>
            </View>
            {data.authorRightsSpecialConditions ? (
              <Text style={[styles.sectionContent, { marginTop: 8 }]}>{data.authorRightsSpecialConditions}</Text>
            ) : null}
          </View>
        )}
        ...
        <View style={styles.summary}>
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>{formatCurrency(subtotal * 100, data.currency)}</Text>
          </View>
          ...
          {isAuthorRightsInvoice && authorRightsData ? (
            <>
              ...
              <View style={styles.taxRow}>
                <Text style={styles.summaryLabel}>VAT total</Text>
                <Text style={styles.summaryValue}>{formatCurrency(authorRightsData.totalVatAmountCents, data.currency)}</Text>
              </View>
              <View style={styles.taxRow}>
                <Text style={styles.summaryLabel}>Withholding tax</Text>
                <Text style={styles.summaryValue}>-{formatCurrency(authorRightsData.withholdingAmountCents, data.currency)}</Text>
          ...
          <View style={styles.totalRow}>
            <Text style={styles.totalLabel}>{isAuthorRightsInvoice ? "Net payable" : data.summaryTotalLabel}</Text>
            <Text style={styles.totalValue}>{formatCurrency(total * 100, data.currency)}</Text>
          </View>
          {isAuthorRightsInvoice && (
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>VAT gross</Text>
              <Text style={styles.summaryValue}>{formatCurrency(Math.round(taxTotal * 100), data.currency)}</Text>
            </View>
          )}
        </View>
